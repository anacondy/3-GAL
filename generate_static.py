...
def generate_static_html(announcements):
    ...
    # Replace the Jinja2 template section with static content
    # Find and replace the for loop section
    static_html = template.replace(
        "{% for item in initial_data %}",
        "<!-- Static content generated by GitHub Actions -->"
    )
    
    # Remove the endfor
    static_html = static_html.replace("{% endfor %}", "")
    
    # Remove the if/endif for empty data
    static_html = static_html.replace('{% if initial_data|length == 0 %}', '<!-- ')
    static_html = static_html.replace('{% endif %}', ' -->')
    
    # Replace the template card with our generated cards
    # Find the exam-card div in the template and replace
    card_pattern = r'<div class="exam-card[^>]*data-url[^>]*>.*?</div>\s*</div>\s*</div>'
    
    # Simpler approach: just inject the cards after the results-list opening
    results_list_start = '<div class="results-container" id="results-list">'
    if results_list_start in static_html:
        # Find where to inject
        idx = static_html.find(results_list_start) + len(results_list_start)
        
        # Find the closing div for results-container
        # Count divs to find the right closing tag
        depth = 1
        end_idx = idx
        while depth > 0 and end_idx < len(static_html):
            if static_html[end_idx:end_idx+5] == '<div ':
                depth += 1
            elif static_html[end_idx:end_idx+6] == '</div>':
                depth -= 1
            end_idx += 1
        
        # Remove old content between results-list and its closing div
        # Find the closing </div> for results-container
        close_div_idx = static_html.rfind('</div>', 0, static_html.find('</div>', idx) + 100)
        
    # Alternative: Create a completely new static page
...
def generate_full_static_html(announcements):
    ...
    if announcements:
        for item in announcements:
            ...
            cards_html += f'''
            <div class="exam-card" onclick="openPdf('{url}')">
                <div class="card-header">
                    <div>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        @media screen and (max-width: 480px) {{
            ...
            .container {{ width: 95%; margin-top: 2vh; }}
